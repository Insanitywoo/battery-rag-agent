  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(payload?.message || payload?.detail || "Registration failed.");
      }
    ...
    <div className="mx-auto flex min-h-[calc(100vh-73px)] max-w-6xl items-center px-6 py-16">
      <div className="grid w-full gap-10 lg:grid-cols-[0.85fr_1.15fr]">
        <section className="rounded-[2rem] border border-slate-200 bg-white/90 p-8 shadow-xl shadow-slate-200/40">
          ...
          <h1 className="mt-4 text-4xl font-semibold text-ink">Create your account</h1>
          <p className="mt-4 text-base leading-8 text-slate-600">
            This account becomes the owner boundary for your personal research projects.
          </p>
          <div className="mt-8 rounded-2xl border border-slate-200 bg-mist p-5 text-sm leading-7 text-slate-600">
            Passwords are sent to the backend for hashing and are never stored in
            browser storage by the frontend.
          </div>
        </section>
        ...
        <section className="rounded-[2rem] border border-slate-200 bg-white/90 p-8 shadow-xl shadow-slate-200/40">
          ...
          <p className="mt-6 text-sm text-slate-600">
            Already registered?{" "}
            <Link href="/login" className="font-medium text-cobalt hover:text-blue-700">
              Sign in
            </Link>
          </p>
        </section>
